...
inputs:
- doc: The combined GVCF output file  Required.
  id: output
  inputBinding:
    position: 0
    prefix: --output
...
- doc: 'One or more specific annotations to exclude from variant calls  This argument
    ...
    ChromosomeCounts, Coverage, DepthPerAlleleBySample, ExcessHet, FisherStrand, InbreedingCoeff,
    MappingQualityRankSumTest, QualByDepth, ReadPosRankSumTest, RMSMappingQuality,
    StrandOddsRatio}'
...
- doc: 'If > 0, reference bands will be broken up at genomic positions that are multiples
    of this number  Default value: 0.'
  id: break_bands_at_multiples_of
  inputBinding:
    position: 0
    prefix: --break-bands-at-multiples-of
  type: int?
...
- doc: 'Size of the cloud-only prefetch buffer (in MB; 0 to disable).  Default value:
  ...
  type: int?
- doc: 'If specified, convert banded gVCFs to all-sites gVCFs  Default value: false.
    Possible values: {true, false}'
  id: convert_to_base_pair_resolution
  inputBinding:
    position: 0
    prefix: --convert-to-base-pair-resolution
  type: boolean?
...
- doc: 'Use all possible annotations (not for the faint of heart)  Default value:
  ...
  type: boolean?
- doc: 'Restrict variant output to sites that start within provided intervals (only
    applies when an interval is specified)  Default value: false. Possible values:
    {true, false}'
