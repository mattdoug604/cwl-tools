...
cwlVersion: v1.0
id: gatk_RevertOriginalBaseQualitiesAndAddMateCigar
inputs:
- doc: The input SAM/BAM file to revert the state of.  Required.
  ...
  inputBinding:
    ...
    prefix: --INPUT
  type: File
- doc: The output SAM/BAM file to create.  Required.
  id: OUTPUT
  inputBinding:
    position: 0
    prefix: --OUTPUT
...
- doc: 'When writing files that need to be sorted, this will specify the number of
  ...
  type: int?
- doc: 'The maximum number of records to examine to determine if we can exit early
    and not output, given that there are a no original base qualities (if we are to
    restore) and mate cigars exist. Set to 0 to never skip the file.  Default value:
    10000.'
...
- doc: 'True to restore original qualities from the OQ field to the QUAL field if
  ...
  type: boolean?
- doc: ':SortOrder    The sort order to create the reverted output file with.By default,
    the sort order will be the same as the input.  Default value: null. Possible values:
