...
id: gatk_Funcotator
inputs:
- doc: The path to a data source folder for Funcotator.  May be specified more than
    once to handle multiple data source folders.  This argument must be specified
    at least once. Required.
  id: data_sources_path
  inputBinding:
    position: 0
    prefix: --data-sources-path
  type: string
- doc: Output VCF file to which annotated variants should be written.  Required.
  id: output
  inputBinding:
    position: 0
    prefix: --output
  type: File
- doc: ':OutputFormatType The output file format.  Either VCF or MAF.  Please note
    that MAF output for germline use case VCFs is unsupported.  Required. Possible
    values: {VCF, MAF}'
  ...
  type: string
- doc: The version of the Human Genome reference to use (e.g. hg19, hg38, etc.).  This
    will correspond to a sub-folder of each data source corresponding to that data
    source for the given reference.  Required.
  id: ref_version
  inputBinding:
    position: 0
    prefix: --ref-version
...
- doc: 'If true, adds a command line header line to created VCF files.  Default value:
  ...
  type: boolean?
- doc: 'Annotations to include in all annotated variants if the annotation is not
    specified in the data sources (in the format <ANNOTATION>:<VALUE>).  This will
    add the specified annotation to every annotated variant if it is not already present.  This
  ...
  type: string?
- doc: 'Override values for annotations (in the format <ANNOTATION>:<VALUE>).  Replaces
    existing annotations of the given name with given values.  This argument may be
    specified 0 or more times. Default value: null.'
  id: annotation_override
  inputBinding:
    position: 0
...
- doc: 'If specified, do not check the sequence dictionaries from our inputs for compatibility.
  ...
  type: boolean?
- doc: 'Fields that should not be rendered in the final output.  Only exact name matches
    will be excluded.  This argument may be specified 0 or more times. Default value:
    null.'
  id: exclude_field
  inputBinding:
    position: 0
...
- doc: 'Lenient processing of VCF files  Default value: false. Possible values: {true,
  ...
  inputBinding:
    ...
    prefix: --lenient
  type: boolean?
- doc: 'Number of base-pairs to cache when querying variants.  Default value: 100000.'
  id: lookahead_cache_bp
  inputBinding:
    position: 0
...
- doc: ':ValidationStringency Validation stringency for all SAM/BAM/CRAM/SRA files
  ...
  type: string?
- doc: 'Ignore/drop variants that have been filtered in the input.  These variants
    will not appear in the output file.  Default value: false. Possible values: {true,
    false}'
  id: remove_filtered_variants
  inputBinding:
    position: 0
...
- doc: 'Temp directory to use.  Default value: null.'
  ...
  inputBinding:
    ...
    prefix: --tmp-dir
  type: string?
- doc: 'to use as a list of transcripts (one transcript ID per line, version numbers
    are ignored) OR A set of transcript IDs to use for annotation to override selected
    transcript. This argument may be specified 0 or more times. Default value: null.'
  id: transcript_list
  inputBinding:
    position: 0
    prefix: --transcript-list
  type: File?
- doc: ':TranscriptSelectionMode Method of detailed transcript selection.  This will
    select the transcript for detailed annotation (CANONICAL, ALL, or BEST_EFFECT).  Default
    value: CANONICAL. Possible values: {BEST_EFFECT, CANONICAL, ALL}'
