...
cwlVersion: v1.0
id: gatk_LiftOverIntervalList
inputs:
- doc: picard.jar LiftOverIntervalList \
  id: jar
  inputBinding:
    position: 0
    prefix: -jar
...
- doc: Chain file that guides the LiftOver process.  Required.
  ...
  inputBinding:
    ...
    prefix: --CHAIN
  type: File
- doc: The input interval list to be lifted over.  Required.
  id: INPUT
  inputBinding:
    position: 0
    prefix: --INPUT
  type: File
- doc: The output interval list file.  Required.
  id: OUTPUT
  inputBinding:
    position: 0
    prefix: --OUTPUT
  type: File
- doc: :FileSequence dictionary to place in the output interval list. (This should
    be the dictionary of the target reference.)  Required.
...
- doc: 'When writing files that need to be sorted, this will specify the number of
  ...
  type: int?
- doc: 'Minimum percentage of bases in each input interval that must map to output
    interval for liftover of that interval to occur. If the program fails to find
    a good target for an interval, a warning will be emitted and the interval will
    be dropped from the output. Default value: 0.95.'
  id: MIN_LIFTOVER_PCT
  inputBinding:
    position: 0
